...
// MAGIC 
// MAGIC This assignment focusses on Problem Scope 2
// MAGIC 
...
// MAGIC %md First we explore - if the Adult is using Coding in his personal life

// COMMAND ----------

// MAGIC %sql 
// MAGIC select tar_2, count(X22) as Coder_for_life  from HackData_2 group by tar_2 order by tar_2;

// COMMAND ----------

// MAGIC %md Next we explore - how likely will the adult update their resumes

// COMMAND ----------

// MAGIC %sql 
// MAGIC select tar_2, count(X39) as Is_Applicant from HackData_2 group by tar_2 order by tar_2;
...
val assembler = new VectorAssembler().setInputCols(Array("X1","X10","X13","X15","X19","X20","X21","X22","X25","X26","X27","X28","X29","PLE")).setOutputCol("features")
...
val rnf_model = new RandomForestClassifier().setLabelCol("Tar_2").setFeaturesCol("scaledFeatures")
val lgbm_model = new LightGBMClassifier().setLabelCol("Tar_2").setFeaturesCol("scaledFeatures")
val svc_model = new LinearSVC().setLabelCol("Tar_2").setFeaturesCol("scaledFeatures")
val log_model = new LogisticRegression().setLabelCol("Tar_2").setFeaturesCol("scaledFeatures")
...
val train_pred_rnf = fit_rnf.transform(scaler_training).selectExpr("Prediction", "Tar_2",
    """CASE Prediction = Tar_2
  WHEN true then 1
  ELSE 0
END as equal""")
...
val train_pred_lgbm = fit_lgbm.transform(scaler_training).selectExpr("Prediction", "Tar_2",
    """CASE Prediction = Tar_2
  WHEN true then 1
  ELSE 0
END as equal""")
...
val train_pred_log = fit_log.transform(scaler_training).selectExpr("Prediction", "cast(Tar_2 as Double) Tar_2",
    """CASE Prediction = Tar_2
  WHEN true then 1
  ELSE 0
END as equal""")
...
val holdout_test_log = fit_log
  .transform(scaler_test)
  .selectExpr("Prediction", "cast(Tar_1 as Double) Tar_2",
    """CASE Prediction = X47
  WHEN true then 1
  ELSE 0
END as equal""")
...
// MAGIC %md
// MAGIC <pre>
// MAGIC 0.102553907	adult-Use coding/tech in my personal life				
// MAGIC 0.09766005	adult-Take on new coding/tech tasks at work				
// MAGIC 0.093453279	adult-Updated resume/LinkedIn to include coding/tech skills				
// MAGIC 0.089976481	adult-Find new job opportunity through the CLC job board				
// MAGIC </pre>
